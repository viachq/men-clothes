...
def test_cart(test_db, test_user):
    """Create a test cart in the database."""
...
@pytest.fixture
def test_cart_item(test_db, test_cart):
...
def mock_catalog_client():
    """Mock catalog client to avoid external HTTP calls."""
    with patch('backend.clients.catalog_client.get_catalog_client') as mock_get_client:
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        
        def get_product(product_id):
            products = {
                1: {"id": 1, "name": "Test Product", "price": 1000},
                2: {"id": 2, "name": "Another Product", "price": 2000},
            }
        ...
        mock_client.get_product = get_product
        yield mock_client
